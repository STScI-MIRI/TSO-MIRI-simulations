...
# the token target is WASP-62 a F7 star, Kmag = 8.994 (or 20e3 microJy at 2 um, BBT = 6230 K (from the exoplanet catalog))

def star_scene(scene_file=None, write_cube=False):
	...
	bg = Background(level='high', gradient=0., pa=0.)
	scene = star + bg    

	if write_cube:
		scene.writecube(cubefits='lrs_tso_scene.fits', FOV=fov, spatsampling=spat_samp, wrange=[5., 15.], wsampling=0.05,  clobber=True, time=0.0)
